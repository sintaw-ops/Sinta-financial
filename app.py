...
def _conn():
    ...
    try:
        ...
        get_conn.clear()          # buang koneksi mati dari cache
        return get_conn()
...
def save_transactions(df: pd.DataFrame) -> tuple[int, int]:
    ...
    with conn.cursor() as c:
        ...
        inserted = c.rowcount
    conn.commit()
    load_all_transactions.clear()      # invalidasi selektif, bukan clear() global
...
def extract_pdf_data(file, password, bank_type):
    ...
    import pdfplumber   # lazy import: hanya saat user benar-benar upload PDF
    transactions = []
    try:
...
def tab_dashboard(df_all: pd.DataFrame):
    ...
    fig_line = go.Figure()
    fig_line.add_trace(go.Scatter(x=line_data_exp[time_group], y=line_data_exp["amount"],
                                  mode='lines+markers', name='Pengeluaran',
                                  line=dict(color='#e74c3c', width=3)))
    fig_line.add_trace(go.Scatter(x=line_data_inc[time_group], y=line_data_inc["amount"],
                                  mode='lines+markers', name='Pemasukan',
                                  line=dict(color='#2ecc71', width=3)))
    fig_line.update_layout(plot_bgcolor="rgba(0,0,0,0)", hovermode="x unified",
...
def tab_ingestion():
    ...
    with tab_pdf:
        ...
        with st.form("pdf_form", clear_on_submit=True):
            ...
            pdf_password = st.text_input("Password PDF (Jika dokumen dikunci)", type="password",
        ...
        with st.form("manual_form", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 Simpan Manual"):
                manual_df = pd.DataFrame([{
                    ...
                    "amount": m_amount, "type": m_type, "sub_category": m_sub,
                    "category": sub_to_cat(m_sub, m_type), "pocket": m_pocket,
                }])
...
def tab_validation(df_all: pd.DataFrame):
    ...
    st.dataframe(df_all[["date", "description", "amount", "type", "sub_category", "pocket"]],
...
def main():
    st.set_page_config(page_title="CFO Console 6.0", page_icon="🏦",
                       layout="wide", initial_sidebar_state="expanded")
    init_db()   # di-cache: hanya jalan sekali per sesi server
