...
@st.cache_resource
def get_conn():
    """Koneksi tunggal yang di-cache (1x per sesi server, bukan per rerun)."""
    import psycopg2
    db_uri = (
...
def _conn():
    """Ambil koneksi cached; reconnect otomatis jika terputus (idle timeout Supabase)."""
    conn = get_conn()
    try:
        if conn.closed:
...
@st.cache_resource
def init_db():
    """Jalankan SEKALI per sesi server (di-cache), bukan tiap rerun."""
    conn = _conn()
    with conn.cursor() as c:
        c.execute("""
...
def load_categories_df():
    return pd.read_sql_query("SELECT * FROM categories", _conn())
...
def save_transactions(df: pd.DataFrame) -> tuple[int, int]:
    """Batch insert (1 round-trip) menggantikan loop per-baris."""
    conn = _conn()
    rows = []
...
def extract_pdf_data(file, password, bank_type):
    """Ekstraksi baris transaksi PDF. pdfplumber di-import lazy agar cold start cepat."""
    import pdfplumber
    transactions = []
    try:
...
def _plotly_base() -> dict:
    """Layout dasar Plotly yang konsisten."""
    return dict(
...
def tab_dashboard(df_all: pd.DataFrame):
    ...
    fig_line.update_layout(**_plotly_base())
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 6 — UI: BUDGET VS ACTUAL (NEW)
# ══════════════════════════════════════════════════════════════════════════════
...
def tab_budget_vs_actual(df_all: pd.DataFrame):
    ...
    with st.container(border=True):
        ...
        avail_years = sorted(df_exp["year"].unique(), reverse=True)
        ...
        sel_months   = col2.multiselect(
            "Filter Bulan",
            avail_months, default=avail_months,
            format_func=lambda m: datetime(2000, m, 1).strftime("%B"),
        )
    ...
    # ── KPI ──────────────────────────────────────────────────────────────────
    total_budget = MONTHLY_BUDGET * n_months
    ...
    parent_df = load_categories_df()[load_categories_df()["tx_type"] == "Expense"]
    parent_budget = (
        parent_df.groupby("parent_category")["monthly_budget"].sum() * n_months
    ).reset_index()
    ...
    for _, row in merged.iterrows():
        pct = min((row["actual"] / row["budget"] * 100), 100) if row["budget"] > 0 else 0
        over = row["actual"] > row["budget"]
        color = "#E24B4A" if over else ("#f39c12" if pct > 85 else "#639922")
        label_color = "🔴" if over else ("🟡" if pct > 85 else "🟢")
        col_a, col_b = st.columns([3, 1])
        col_a.markdown(f"**{row['category']}** {label_color}")
        col_b.markdown(
            f"`{fmt_idr(row['actual'])} / {fmt_idr(row['budget'])}` — **{pct:.1f}%**",
    ...
    # Budget harian = total budget / jumlah hari dalam periode
    import calendar
    n_days_list = [calendar.monthrange(sel_year, m)[1] for m in sel_months]
    total_days  = sum(n_days_list)
    ...
    sel_cats = st.multiselect(
        ...
        help="Pilih kategori yang ingin ditampilkan di chart",
    )
    ...
    if sel_cats:
        ...
        st.info("Pilih minimal satu kategori untuk menampilkan chart.")
    ...
    summary_display = summary[["sub_category", "budget", "actual", "selisih", "pct", "status"]].copy()
    summary_display.columns = ["Sub-Kategori", "Budget (IDR)", "Actual (IDR)", "Selisih (IDR)", "% Realisasi", "Status"]
    for col in ["Budget (IDR)", "Actual (IDR)", "Selisih (IDR)"]:
        summary_display[col] = summary_display[col].apply(fmt_idr)
    summary_display["% Realisasi"] = summary_display["% Realisasi"].apply(lambda v: f"{v:.1f}%")
    st.dataframe(summary_display, use_container_width=True, hide_index=True)

# ══════════════════════════════════════════════════════════════════════════════
# SECTION 7 — UI: INGESTION DATA
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab_pdf:
        ...
        with st.form("pdf_form", clear_on_submit=True):
            ...
            pdf_password = st.text_input(
                "Password PDF (Jika dokumen dikunci)", type="password",
                help="Contoh: 23092000 untuk Sinarmas",
            )
...
def main():
    ...
    nav_selection = st.sidebar.radio(
        "Navigasi Utama",
        ["📊 Dashboard", "🎯 Budget vs Actual", "⚙️ Validasi Antrean", "📥 Ingestion Data"],
        key="current_nav",
    )
